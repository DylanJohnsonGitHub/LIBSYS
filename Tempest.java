...
//-----------MENU SET UP--------------------------//
		...
		*/
		JMenuItem searchBooks = new JMenuItem("Check Out/Edit/Remove", KeyEvent.VK_U);
...
//------------------DEFAULT PANEL SETUP-----------------------------------------------------------------//
		...
		JLabel defaultLabel = new JLabel("<html>WELCOME TO LIBSYS!<br><h3><center>Please use the Menu to Navigate the System!</center></h3></span></html>");
...
//---------------------------SEARCH PANEL SETUP--------------------------------------//
		...
		JLabel searchCatalogLabel = new JLabel("Search the catalog for a book to check out, edit, or remove");
		...
		searchPanel.add(BorderLayout.NORTH, searchCatalogLabelPanel);
		searchPanel.add(BorderLayout.CENTER, searchLabelPanel);
		searchPanel.add(BorderLayout.SOUTH, searchSearchingPanel);
...
//------------------------SEARCH POPUP FRAME SETUP-------------------------------------//
		...
		JButton searchEditButton  = new JButton("Edit Highlighted Result");
		JButton searchRemoveButton  = new JButton("Remove Highlighted Result");
		
		JPanel searchWindowButtonPanel = new JPanel();
		
		searchWindowButtonPanel.add(searchSelectButton);
		searchWindowButtonPanel.add(searchEditButton);
		searchWindowButtonPanel.add(searchRemoveButton);
		...
		/*
		 * bookListModel.
		 * addElement("<html><center>----------------------------------<br><h1>Test Title</h1><br>Mary had a little lamb<br>----------------------------------------------</center></span></html>"
		 * );
		 * bookListModel.addElement("<html>Mary had a<br> little lamb</span></html>");
		 * bookListModel.
		 * addElement("<html>Mary<br> had<br> a<br> little<br> lamb</span></html>");
		 * bookListModel.
		 * addElement("<html>1234<br>Test Book Title<br>Test Author</span></html>");
		 * bookListModel.addElement(LocalDate.now());
		 */
		
		JScrollPane bookListScroller = new JScrollPane(bookList);
		
		searchCatalogFrame.add(BorderLayout.NORTH, searchWindowButtonPanel);
		...
		searchCatalogFrame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
...
//-----------------------MANAGE MEMBER FRAME SETUP------------------------------------//
		...
		JButton manageUserSearchButton = new JButton("Update Member Info");
...
//----------------------EDIT CATALOG PANEL SETUP---------------------------------------//
		
		createBookPanel.setLayout(new BorderLayout());
		
		JPanel createBookSouthPanel = new JPanel();
		 createBookSouthPanel.setLayout(new BorderLayout());
		JPanel createBookInfoPanel = new JPanel();
		createBookInfoPanel.setLayout(new BorderLayout());
		JPanel createBookCenterPanel = new JPanel();
		createBookCenterPanel.setLayout(new BorderLayout());
		
		JPanel createBookTitlePanel = new JPanel();
		JLabel createBookTitleLabel = new JLabel("Title: ");
		JTextField createBookTitleText = new JTextField(10);
		createBookTitlePanel.add(createBookTitleLabel);
		createBookTitlePanel.add(createBookTitleText);
		
		JPanel createBookAuthorPanel = new JPanel();
		JLabel createBookAuthorLabel = new JLabel("Author: ");
		JTextField createBookAuthorText = new JTextField(10);
		createBookAuthorPanel.add(createBookAuthorLabel);
		createBookAuthorPanel.add(createBookAuthorText);
		
		JPanel createBookISBNPanel = new JPanel();
		JLabel createBookISBNLabel = new JLabel("ISBN: ");
		JTextField createBookISBNText = new JTextField(10);
		createBookISBNPanel.add(createBookISBNLabel);
		createBookISBNPanel.add(createBookISBNText);
		
		createBookInfoPanel.add(BorderLayout.NORTH, createBookISBNPanel);
		createBookInfoPanel.add(BorderLayout.CENTER, createBookTitlePanel);
		createBookInfoPanel.add(BorderLayout.SOUTH, createBookAuthorPanel);
		
		JPanel createBookLabelPanel = new JPanel();
		JLabel createBookLabel = new JLabel("(Create Book)");
		createBookLabelPanel.add(createBookLabel);
		
		JPanel createBookButtonPanel = new JPanel();
		JButton createBookButton = new JButton("Create");
		createBookButtonPanel.add(createBookButton);
		
		JPanel createBookAreaPanel = new JPanel();
		JTextArea createBookArea = new JTextArea("[Resulting Database Entry Here]");
		createBookArea.setSize(100, 100);
		createBookAreaPanel.add(createBookArea);
		
		JPanel createBookUpdatePanel = new JPanel();
		JButton createBookUpdateButton = new JButton("Update Database Entry Preview");
		createBookUpdatePanel.add(createBookUpdateButton);
		
		createBookCenterPanel.add(BorderLayout.NORTH, createBookUpdatePanel);
		createBookCenterPanel.add(BorderLayout.SOUTH, createBookAreaPanel);
		
		createBookSouthPanel.add(BorderLayout.NORTH, createBookButtonPanel);
		createBookSouthPanel.add(BorderLayout.SOUTH, createBookLabelPanel);
		
		createBookPanel.add(BorderLayout.NORTH, createBookInfoPanel);
		createBookPanel.add(BorderLayout.CENTER, createBookCenterPanel);
		createBookPanel.add(BorderLayout.SOUTH, createBookSouthPanel);
...
//---------------------------FRAME DISPLAY--------------------------------------------//
		 ...
		 * 
		 * //checks to see if the panel array has been looped through if(i <
		 * panelArray.length - 1) { //if so, increments i i++;
		 * 
		 * }
		 * 
		 * else { //otherwise, resets i to beginning of loop i = 0;
		 * 
		 * }
		 * 
		 * //changes the current panel //removes everything from the current frame
		 * libFrame.getContentPane().removeAll(); //adds the ith panel according to the
		 * button presses libFrame.getContentPane().add(BorderLayout.NORTH,
		 * panelArray[i]); //adds the foot panel with the time, date, and increment
		 * button libFrame.getContentPane().add(BorderLayout.SOUTH, footPanel);
		 * //ensures everything is added to frame, repacks it, and sets it visible
		 * libFrame.revalidate(); libFrame.pack();
		 * 
		 * }
		 * 
		 * });
...
//--------------------------------FUNCTION-------------------------------------------------------------------//
		...
		searchSearchButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(!searchISBN.isEmpty() || !searchTitle.isEmpty() || !searchAuthor.isEmpty()) {
					...
					driver.getBooksFromDB(searchISBN, searchTitle, searchAuthor);
					String bookStrings[] = driver.printBooks();
					for (String curBook: bookStrings)
						bookListModel.addElement(curBook);
		...
		searchSelectButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//gets the book from the local
				driver.selectBookFromLocal(bookList.getSelectedIndex());
				searchCatalogFrame.dispose();
				String temp[] = driver.getCurrentBookInfo();
				bookNumText.setText(temp[0]);
				bookTitleText.setText(temp[1]);
				bookAuthorText.setText(temp[2]);
				switcher.panelSwap(1);
		...
		/*
		checkoutBooks.addActionListener(new ActionListener() {
		...
		*/
		memberReturn.addActionListener(new ActionListener() {
			
			public void actionPerformed(ActionEvent e) {
				
				switcher.panelSwap(2);

			}
		
		});
		
		updateMemberInfo.addActionListener(new ActionListener() {
			
			public void actionPerformed(ActionEvent e) {
				
				switcher.panelSwap(4);
		...
		createBookButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				driver.addBookToDB(createBookISBNText.getText(),
								   createBookTitleText.getText(),
								   createBookAuthorText.getText(), 
								   1);
